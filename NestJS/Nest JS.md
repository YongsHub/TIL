...
[인용한 사이트](https://wikidocs.net/158650)

```
Nest는 프레임워크 내에 예외 레이어를 두고 있습니다. 애플리케이션을 통틀어 제대로 처리하지 못한 예외를 처리하는 역할을 합니다. 여러분이 아무런 작업을 하지 않아도 기본 예외 처리기가 예외를 잡아서 유저가 이해하기 쉬운 형태로 변환하여 전송합니다

Client 입장에서 공통된 Response를 넘겨줘야 Flow를 처리하기 편하기 때문에 적절한 Custom을 하여 사용할 수 있다.
```
